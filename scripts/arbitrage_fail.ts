...
async function main() {
    ...
    await doTransaction(publicClient, await tokenB.write.approve([dex.address, 100n]))
    await doTransaction(publicClient, await dex.write.swapB([100n]))
    ...
    // // very high slippage, will fail
    // await doTransaction(publicClient, await tokenA.write.approve([arbitrage.address, 1000n]));
    // const hash = await arbitrage.write.execute([1000n, 0]);
    // await doTransaction(publicClient, hash);
    // console.log(`Unsuccesful arbitrage: ${hash}`)
